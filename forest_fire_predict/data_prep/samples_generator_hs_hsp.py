...
def calculate_dgi(current_date, previous_date):
    DGI = (current_date - previous_date).days
    return DGI if (DGI>=0) else 0
...
def generate_dataset_hs_hsp():
    ...
    meta_df = pd.DataFrame(columns=['SAMPLE_INDEX','fireID','NFIREID','YEAR','AGENCY','FIRECAUS','ADJ_HA','bDays','sDate','eDate','is_mp', 'fireDate', 'DGI'])
    # sample count
    sample_index = 0
    ...
    for year in years:
        ...
        for fire_instance_folder in os.listdir(yearly_instances_path):
            ...
            # set previous date record for DGI calculation
            ...
            for date in fire_timerange:
                ...
                if os.path.isfile(todays_hs) and os.path.isfile(tmrs_hs) and (not contains_NaN(todays_fwi)):
                    print(f"found {sample_index}th sample on date: {str(date)}")
                    ...
                    # calculate DGI
                    DGI = calculate_dgi(date, previous_date)
                    previous_date = date
                    ...
                    # create DGI file
                    np.save(f'{current_sample_path}/{sample_index} DGI.npy', DGI)

                    # update dataframe 'SAMPLE_INDEX','fireID','NFIREID','YEAR','AGENCY','FIRECAUS','ADJ_HA','bDays','sDate','eDate','is_mp'
                    new_row_data = ([sample_index, fire_instance_folder] + ba_row[['NFIREID', 'YEAR', 'AGENCY', 'FIRECAUS', 'ADJ_HA', 'bDays','sDate','eDate','is_mp']].tolist()+[date, DGI])
