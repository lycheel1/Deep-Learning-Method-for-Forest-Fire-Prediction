...
def fill_nan_with_neighbor_average(old_array):
        ...
        for x in [i-1, i, i+1]:
            for y in [j-1, j, j+1]:
                if x >= 0 and x < array.shape[0] and y >= 0 and y < array.shape[1] and (x != i or y != j):
                    if not np.isnan(array[x, y]):
                        neighbors.append(array[x, y])
        if neighbors:
            array[i, j] = np.mean(neighbors)
        else:
            print(f'NaN at all the neighbors')
    ...
    for year in years:
        ...
        for fire_instance_folder in os.listdir(yearly_instances_path):
            ...
            if np.isnan(elevation).any():
                ...
                new_elevation = fill_nan_with_neighbor_average(elevation)

                np.save(f'{fire_instance_path}/{fire_instance_folder} elev_nan.npy', elevation)
                np.save(file_path, new_elevation)

            # in consideration of repetitively running
            if os.path.isfile(f'{fire_instance_path}/{fire_instance_folder} elev_nan.npy'):
                year_list.append(year)
                instance_list.append(fire_instance_folder)


    df = pd.DataFrame({'year': year_list, 'instance': instance_list})
    df.to_csv('elev_nan_check.csv', index=False)