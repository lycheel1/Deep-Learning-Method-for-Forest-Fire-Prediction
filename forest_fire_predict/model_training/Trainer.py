...
from util.matrices_computation import calculate_metrics, calculate_AUC
...
class WF01_Trainer():
    ...
    def _train_model(self, train_loader):
        ...
        acc_train, pre_train, rec_train, spe_train, f1_train, cm_train = calculate_metrics(all_outputs_train,
                                                                                           all_labels_train)
        roc_auc_train, pr_auc_train = calculate_AUC(all_outputs_train,all_labels_train)
        average_loss_train = total_loss / len(train_loader)

        return average_loss_train, acc_train, pre_train, rec_train, spe_train, f1_train, roc_auc_train, pr_auc_train, cm_train
    ...
    def _val_model(self, val_loader):
        ...
        roc_auc_val, pr_auc_val = calculate_AUC(all_outputs_val, all_labels_val)

        average_loss_val = total_loss / len(val_loader)

        return average_loss_val, acc_val, pre_val, rec_val, spe_val, f1_val, roc_auc_val, pr_auc_val, cm_val
    ...
    def K_fold_train_and_val(self, model_save_path, model_save_name):
        ...
        for fold, (train_ids, val_ids) in enumerate(kf.split(self.train_set)):
            ...
            for epoch in range(self.epochs):
                ...
                train_loss, train_acc, train_pre, train_rec, train_spe, train_f1, train_roc_auc, train_pr_auc, train_cm = self._train_model(train_loader)
                val_loss, val_acc, val_pre, val_rec, val_spe, val_f1, val_roc_auc, val_pr_auc, val_cm = self._val_model(val_loader)

                dict_train = self._metrics_dict(fold, epoch, 'train',
                    train_loss, train_acc, train_pre, train_rec, train_spe,
                                                train_f1, train_roc_auc, train_pr_auc, *train_cm)

                dict_val = self._metrics_dict(fold, epoch, 'val',
                                                val_loss, val_acc, val_pre, val_rec, val_spe,
                                              val_f1, val_roc_auc, val_pr_auc, *val_cm)
                ...
                print(f'Train Loss {train_loss:.6f}, Acc: {train_acc*100:.4f}%, Pre: {train_pre*100:.4f}%, Rec: {train_rec*100:.4f}%, Spe: {train_spe*100:.4f}%, '
                      f'ROC_AUC: {train_roc_auc:.6f}, PR_AUC: {train_pr_auc:.6f}, F1: {train_f1*100:.6f}%')
                print(f'Val   Loss {val_loss:.6f}, Acc: {val_acc*100:.4f}%, Pre: {val_pre*100:.4f}%, Rec: {val_rec*100:.4f}%, Spe: {val_spe*100:.4f}%, '
                      f'ROC_AUC: {val_roc_auc:.6f}, PR_AUC: {val_pr_auc:.6f}, F1: {val_f1*100:.6f}%')
    ...
    def test_model(self, reload, metrics_save_path=None, model_weights_path=None, fold=None, epoch=None):
        ...
        # updated AUC calculation #
        roc_auc, pr_auc = calculate_AUC(all_outputs_test, all_labels_test)

        # Calculate average loss and accuracy
        test_loss = total_loss / len(test_loader)

        dict_test = self._metrics_dict(fold, epoch, 'test',
                    test_loss, acc_test, pre_test, rec_test, spe_test, f1_test, roc_auc, pr_auc, *cm_test)
        ...
        print(f'Test  Loss {test_loss:.6f}, Acc: {acc_test*100:.4f}%, Pre: {pre_test*100:.4f}%, Rec: {rec_test*100:.4f}%, Spe: {spe_test*100:.4f}%, '
              f'ROC_AUC: {roc_auc:.6f}, PR_AUC: {pr_auc:.6f}, F1: {f1_test*100:.6f}')
