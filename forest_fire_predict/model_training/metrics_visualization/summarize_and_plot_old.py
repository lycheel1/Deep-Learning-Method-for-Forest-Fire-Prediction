...
def plot_loss_curve(metrics_path, phase_list, output_path, output_name):
    ...
    test_metrics_at_min_val_loss.to_csv(new_csv_path, index=False)
    return new_csv_path
...
if __name__ == '__main__':
    ...
    for aug_name in aug_list:
        ...
        metrics_path = f'{path}/{model_name}_kfold{aug_name}.csv'
        best_kfold_csv_path = plot_loss_curve(metrics_path, ['train', 'val', 'test'], f'{path}', output_name=f'{model_name}{aug_name}')
        calculate_and_save_avg_metrics(best_kfold_csv_path, path, f'{model_name}{aug_name}')
