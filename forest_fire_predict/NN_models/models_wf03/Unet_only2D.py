...
class Custom_UNet_only2D(nn.Module):

    def __init__(self, in_channels=(5+8-1), out_channels=1, init_features=32, num_1d_features=51):
        super(Custom_UNet_only2D, self).__init__()

        features = init_features

        self.fuel_embedding = Embedding_block(embedding_dim=8)

        self.encoder1 = Custom_UNet_only2D._block(in_channels, features, name="enc1")
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder2 = Custom_UNet_only2D._block(features, features * 2, name="enc2")
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder3 = Custom_UNet_only2D._block(features * 2, features * 4, name="enc3")
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder4 = Custom_UNet_only2D._block(features * 4, features * 8, name="enc4")
        ...
        self.decoder4 = Custom_UNet_only2D._block((features * 8) * 2, features * 8, name="dec4")
        self.upconv3 = nn.ConvTranspose2d(
            features * 8, features * 4, kernel_size=2, stride=2
        )
        self.decoder3 = Custom_UNet_only2D._block((features * 4) * 2, features * 4, name="dec3")
        self.upconv2 = nn.ConvTranspose2d(
            features * 4, features * 2, kernel_size=2, stride=2
        )
        self.decoder2 = Custom_UNet_only2D._block((features * 2) * 2, features * 2, name="dec2")
        self.upconv1 = nn.ConvTranspose2d(
            features * 2, features, kernel_size=2, stride=2
        )
        self.decoder1 = Custom_UNet_only2D._block(features * 2, features, name="dec1")
